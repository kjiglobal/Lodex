...
test('desktop menus, clipboard persistence, temporary images and independent windows', { timeout: 90_000 }, async () => {
  ...
  try {
    ...
    assert(menu.find(item => item.label === 'Help').children.some(item => item.label === 'Lodex ' + require('../package.json').version));
    assert(menu.find(item => item.label === 'Help').children.some(item => item.label === 'Check for Updates…'));
    await first.bringToFront();
    await first.waitForFunction(() => document.hasFocus());
    await instance.evaluate(({ Menu }) => Menu.getApplicationMenu().items.find(item => item.label === 'Help').submenu.items.find(item => item.label === 'About Lodex').click());
    const about = first.getByRole('dialog', { name: 'About Lodex' });
    await about.waitFor();
    assert((await about.innerText()).includes(require('../package.json').version));
    await about.getByRole('button', { name: 'Check for Updates', exact: true }).waitFor();
    const updateState = await first.evaluate(() => window.lodex.updates.state());
    assert.equal(updateState.status, 'idle');
    assert.equal(updateState.currentVersion, require('../package.json').version);
    if (process.platform === 'linux' && process.env.LODEX_TEST_EXECUTABLE) assert.equal(updateState.canInstall, true);
    await assert.rejects(first.evaluate(() => window.lodex.updates.restart()));
    await about.getByTitle('Close update window').click();
