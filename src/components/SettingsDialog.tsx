...
export function SettingsDialog({ theme, onTheme, approvalPolicy, onApproval, instructions, onInstructions, onArchived, onUpdates, onClose }: {
  theme: "light" | "dark"; onTheme(value: "light" | "dark"): void;
  approvalPolicy: string; onApproval(value: string): void;
  instructions: string; onInstructions(value: string): void; onArchived(): void; onUpdates(): void; onClose(): void;
}) {
  ...
  return <div className="modal-backdrop" onClick={event => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="settings-dialog" role="dialog" aria-modal="true" aria-labelledby="settings-title">
      ...
      <div className="settings-row"><span>Updates<small>Check the latest stable Ubuntu release</small></span><button className="secondary-button" onClick={onUpdates}>Check for Updates</button></div>
